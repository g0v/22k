...
class Feed extends CI_Controller {
	...
	{
	    $data['feed_name'] = '揭露22K'; // your website
		$data['encoding'] = 'utf-8'; // the encoding
        $data['feed_url'] = 'http://www.22kopendata.org/feed'; // the url to your feed
        $data['page_description'] = '最新22K職缺列表'; // some description
        $data['page_language'] = 'en-en'; // the language
        $data['creator_email'] = 'open22k@email.com'; // your email
        $data['posts'] = $this->getPosts(10);
    ...
    function list_data($per_page = 10,$page_now = 1)
    {
        ...
        foreach ($query->result() as $row)
        {
            ...
            $data_file_name = $row->data_file_name;
                ...
                //加入item
                $job = xml_add_child($xml_root, 'job');
                xml_add_child($job, 'count', $count);
                xml_add_child($job, 'company_name', $company_name);
                xml_add_child($job, 'company_location', $location);
                xml_add_child($job, 'job_name', $job_name);
